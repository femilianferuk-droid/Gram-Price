...
# УЛУЧШЕНО: добавлен флаг simple_mode для каналов, убирающий лишний текст
def build_price_text(prices: dict, is_live: bool, allowed_assets_str: str = "GRAM,USDT,BTC,DOGE", simple_mode: bool = False) -> str:
    allowed = [a.strip() for a in allowed_assets_str.split(',')]
    text = ""
    
    # Шапка добавляется только если это НЕ для канала
    if not simple_mode:
        dt_now = get_msk_time().strftime("%H:%M:%S")
        status_market = "🟢 Live API" if is_live else "⚠️ Fallback"
    ...
    if not any(coin in allowed for coin in ["GRAM", "USDT", "BTC", "DOGE"]):
        text += "❌ Ни одна монета не выбрана."
...
def settings_menu(target_id: str, is_channel: bool):
    ...
    buttons = []
    if is_channel:
        buttons.append([InlineKeyboardButton(text="🚀 Отправить прайс в канал сейчас", callback_data=f"send_now_{target_id}")])
...
async def cmd_start(message: Message):
    db_execute("INSERT OR IGNORE INTO settings (target_id, is_channel, assets) VALUES (?, 0, 'GRAM,USDT,BTC,DOGE')", (str(message.from_user.id),))
...
async def check_prices_handler(call: CallbackQuery):
    ...
    await call.message.edit_text(build_price_text(prices, is_live, allowed_str, simple_mode=False), reply_markup=prices_keyboard(), parse_mode="HTML")
...
async def set_interval_options(call: CallbackQuery):
    ...
    keyboard = []
    intervals = [("Выкл ❌", 0), ("30 мин", 30), ("1 час", 60), ("4 часа", 240)]
    for text, val in intervals:
        keyboard.append([InlineKeyboardButton(text=text, callback_data=f"saveint_{val}_{target}")])
        
    keyboard.append([InlineKeyboardButton(text="✍️ Ввести свой интервал (в минутах)", callback_data=f"custom_int_{target}")])
    keyboard.append([InlineKeyboardButton(text="🔙 Назад", callback_data=f"back_to_target_{target}")])
    await call.message.edit_text("Выберите фиксированный период или задайте свой собственный:", reply_markup=InlineKeyboardMarkup(inline_keyboard=keyboard))

@router.callback_query(F.data.startswith("custom_int_"))
async def custom_interval_trigger(call: CallbackQuery, state: FSMContext):
    target = call.data.replace("custom_int_", "")
    target_id = str(call.from_user.id) if target == "usr" else target.replace("ch_", "")
    
    await state.update_data(int_target_id=target_id)
    await state.set_state(BotStates.waiting_for_custom_interval)
    await call.message.edit_text("✍️ Напишите в чат интервал отправки сообщений <b>в минутах</b> (например, 15 или 90):", parse_mode="HTML")

@router.message(BotStates.waiting_for_custom_interval)
async def save_custom_interval(message: Message, state: FSMContext):
    try:
        ...
        await message.answer("Пожалуйста, введите корректное целое число больше нуля.")
        return
        
    data = await state.get_data()
    t_id = data['int_target_id']
    
    db_execute("UPDATE settings SET interval_min = ?, last_check = NULL WHERE target_id = ?", (minutes, t_id))
    await message.answer(f"✅ Успешно настроено! Отчеты будут приходить раз в <b>{minutes} мин.</b>", reply_markup=main_menu(), parse_mode="HTML")
    await state.clear()
...
# Отправка в канал СЕЙЧАС (ИСПРАВЛЕНО: simple_mode=True)
@router.callback_query(F.data.startswith("send_now_"))
async def send_now_to_channel(call: CallbackQuery, bot: Bot):
    channel_id = call.data.replace("send_now_", "")
    
    prices, is_live = await fetch_prices()
    res = db_fetchall("SELECT assets FROM settings WHERE target_id = ?", (channel_id,))
    allowed_str = res[0][0] if (res and res[0][0]) else "GRAM,USDT,BTC,DOGE"
    
    # Передаем simple_mode=True, чтобы убрать шапку текста для канала
    text = build_price_text(prices, is_live, allowed_str, simple_mode=True)
    
    try:
        await bot.send_message(chat_id=channel_id, text=text, parse_mode="HTML")
        await call.answer("🚀 Прайс успешно опубликован в канале!", show_alert=True)
    except Exception as e:
        await call.answer(f"❌ Ошибка отправки! Проверьте админ-права бота.\nКод ошибки: {e}", show_alert=True)
...
async def toggle_coin_db(call: CallbackQuery):
    parts = call.data.split("_")
    coin, target_type = parts[1], parts[2]
    target_id = str(call.from_user.id) if target_type == "usr" else "_".join(parts[3:])
...
# =====================================================================
# ТРЕКЕР (ИСПРАВЛЕНО: Авто-отчеты в канал шлются без лишнего текста)
# =====================================================================
async def price_tracker_worker(bot: Bot):
    while True:
        try:
            ...
            # 2. Периодическая рассылка (ИСПРАВЛЕНО: добавлен параметр is_channel)
            settings = db_fetchall("SELECT target_id, interval_min, last_check, assets, is_channel FROM settings WHERE interval_min > 0")
            for target_id, interval, last_check, assets_str, is_channel in settings:
                should_send = False
                if not last_check:
                    should_send = True
                else:
                ...
                if should_send:
                    ...
                    # Если получатель — канал (is_channel == 1), включаем simple_mode=True
                    report_text = build_price_text(prices, is_live, allowed_coins, simple_mode=bool(is_channel))
