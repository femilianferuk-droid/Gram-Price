...
# =====================================================================
# ВРЕМЯ МСК И РЕАЛЬНЫЙ API (Binance + CoinGecko)
# =====================================================================
...
class BotStates(StatesGroup):
    ...
    waiting_for_custom_interval = State() # Новое состояние для ввода минут
...
def settings_menu(target_id: str, is_channel: bool):
    ...
    buttons = []
    # Если это канал — добавляем кнопку принудительной мгновенной отправки
    if is_channel:
...
# Кастомная и фиксированная настройка интервалов
...
async def set_interval_options(call: CallbackQuery):
    ...
    # Кнопка для ручного ввода интервала
...
# Функция мгновенной принудительной отправки в канал (ФИЧА!)
...
async def send_now_to_channel(call: CallbackQuery, bot: Bot):
    ...
    text = build_price_text(prices, is_live, allowed_str)
    
    try:
        await bot.send_message(chat_id=channel_id, text=text, parse_mode="HTML")
        await call.answer("🚀 Прайс успешно опубликован в канале!", show_alert=True)
    except Exception as e:
        await call.answer(f"❌ Ошибка отправки! Проверьте, добавлен ли бот в админы канала.\nКод ошибки: {e}", show_alert=True)
...
async def price_tracker_worker(bot: Bot):
    while True:
        try:
            ...
            # 2. Периодическая отправка отчетов
...
async def main():
    ...
    print("Бот успешно обновлен и готов к деплою на Bothost!")
